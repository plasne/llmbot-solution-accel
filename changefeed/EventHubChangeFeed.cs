...
public class EventHubChangeFeed : IChangeFeed
{
    ...
    private async Task GetEventsFromPartitionAsync(string partitionId, CancellationToken cancellationToken)
    {
        ...
        while (true)
        {
            try
            {
                if (client is null || reset)
                {
                    client = await this.GetOrCreateConsumerClientAsync(reset, cancellationToken);
                }

                reset = false;
                await this.ListenToPartitionAsync(client, partitionId, cancellationToken);
            }
            catch (TaskCanceledException)
            {
                break;
            ...
            {
                logger.LogWarning("the change feed has too many consumers, another will be chosen if possible...");
                reset = true;
            }
            catch (Exception ex)
            {
                logger.LogError(ex, "an exception was thrown in EventHubChangeFeed.GetEventsFromPartition()...");
            }

            if (await DelayBeforeRetryAsync(cancellationToken))
            {
                break;
            }
        }
    ...
    private async Task ListenToPartitionAsync(EventHubConsumerClient client, string partitionId, CancellationToken cancellationToken)
    {
        // listen to events on a partition
        logger.LogInformation("started listening to partition {p} for the change feed...", partitionId);
        await foreach (var evt in client.ReadEventsFromPartitionAsync(partitionId, EventPosition.Latest, cancellationToken))
        {
            if (evt.Data is not null && this.OnNotifiedAsync is not null)
            {
